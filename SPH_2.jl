...
function F_Pressure(pos,v,m,h,k,n)
    ...
    for i in 1:partikelanzahl
        ...
        for j in 1:partikelanzahl
            xj = pos[j,:]
            f_pres[i] = f_pres[i]+m*((P[i] / (rho[i].*rho[i]))+(P[j] / (rho[j].*rho[j])))*dWcub(Abstand(xi,xj),h)
        end
    end
    return f_pres
end
...
function F_Viscosity(pos,v,m,h,k,n,nu)
    ...
    for i in 1:partikelanzahl
        ...
        for j in 1:partikelanzahl
            xj = pos[j,:]
            f_vis[i] = f_vis[i] + m*((v[j,1]-v[i,1])/rho[j])*d2Wcub(Abstand(xi,xj),h)
        end
    end
    f_vis = f_vis .* (nu./rho)
    return f_vis
...
for j in 1:time_steps
    ...
    for i in 1:partikelanzahl
        "a[i,1] = -f_pres[i,1] + f_vis[i,1]
        a[i,2] = -f_pres[i,2] + f_vis[i,2]"
        "v[i,1] = v[i,1] + dt*a
        v[i,2] = v[i,2] + dt*f_pres[i,2]"

        global delta_v = -f_pres .+ f_vis
